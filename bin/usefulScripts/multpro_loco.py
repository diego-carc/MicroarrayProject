...
GSE_IDs = [(id,re.sub(r'^20+', 'GSE', gse)) for id in txid_list for gse in get_GSE_IDs(id, args.max)]
...
start = time.time()
GSE_OBJ_IDs= [(id, obj) for id,gse in GSE_IDs if (obj:=get_GSE_object(gse,path))]
logging.info("Total GEOparsing time %s", format_time(time.time()-start))

# Obtener las tablas de metadatos por txID
logging.info(sep.format("Start metadata parsing"))
start = time.time()
with Pool(args.cores) as p:
  metadata_sync = p.map(get_GEO_metadata_from_txid, GSE_OBJ_IDs)
